...
private object AuthSQL {
  ...
  def insert[A](
      jwt: AugmentedJWT[A, Long]
  )(implicit hs: JWSSerializer[JWSMacHeader[A]]): Update0 = sql"""
    INSERT INTO JWT (
      ID,
      JWT,
      IDENTITY,
  ...
  def select(
      id: SecureRandomId
  ): Query0[(String, Long, Instant, Option[Instant])] = sql"""
    SELECT * FROM JWT WHERE ID = $id
...
object DoobieAuthRepositoryInterpreter {
  def apply[F[_]: Bracket[*[_], Throwable], A](
      key: MacSigningKey[A],
      xa: Transactor[F]
  )(implicit
      hs: JWSSerializer[JWSMacHeader[A]],
