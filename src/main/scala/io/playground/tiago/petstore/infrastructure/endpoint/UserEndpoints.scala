...
import io.playground.tiago.petstore.domain._
import io.playground.tiago.petstore.domain.users._
import io.playground.tiago.petstore.domain.authentication._
...
class UserEndpoints[F[_]: Sync, A, Auth: JWTMacAlgo] extends Http4sDsl[F] {
  ...
  private def updateEndpoint(
      userService: UserService[F]
  ): AuthEndpoint[F, Auth] = { case req @ PUT -> Root / name asAuthed _ =>
    val action = for {
  ...
  private def searchByNameEndpoint(
      userService: UserService[F]
  ): AuthEndpoint[F, Auth] = { case GET -> Root / userName asAuthed _ =>
  ...
  private def deleteUserEndpoint(
      userService: UserService[F]
  ): AuthEndpoint[F, Auth] = { case DELETE -> Root / userName asAuthed _ =>
    for {
