package io.playground.tiago.petstore.infrastructure.endpoint

import io.playground.tiago.petstore.domain.authentication.Auth
import io.playground.tiago.petstore.domain.users.User
import io.playground.tiago.petstore.domain.pets.{Pet, PetService, PetStatus}
...
class PetEndpoints[F[_]: Sync, Auth: JWTMacAlgo] extends Http4sDsl[F] {
  ...
  private def updatePetEndpoint(
      petService: PetService[F]
  ): AuthEndpoint[F, Auth] = { case req @ PUT -> Root / LongVar(_) asAuthed _ =>
    val action = for {
      pet <- req.request.as[Pet]
  ...
  private def getPetEndpoint(
      petService: PetService[F]
  ): AuthEndpoint[F, Auth] = { case GET -> Root / LongVar(id) asAuthed _ =>
  ...
  private def deletePetEndpoint(
      petService: PetService[F]
  ): AuthEndpoint[F, Auth] = { case DELETE -> Root / LongVar(id) asAuthed _ =>
    for {
  ...
  private def getPetsByStatusEndpoint(
      petService: PetService[F]
  ): AuthEndpoint[F, Auth] = {
    case GET -> Root / "getByStatus" :? StatusMatcher(Valid(xs)) asAuthed _ =>
      NonEmptyList.fromList(xs) match {
        case None =>
          BadRequest("status parameter not specified")
        case Some(statuses) =>
          for {
            retrieved <- petService.getByStatus(statuses)
            resp <- Ok(retrieved.asJson)
          } yield resp
      }
  }

  private def getPetsByTagEndpoint(
      petService: PetService[F]
  ): AuthEndpoint[F, Auth] = {
    case GET -> Root / "getByTags" :? TagMatcher(Valid(xs)) asAuthed _ =>
      NonEmptyList.fromList(xs) match {
        case None =>
          BadRequest("tag parameter not specified")
        case Some(tags) =>
          for {
            retrieved <- petService.getByTag(tags)
            resp <- Ok(retrieved.asJson)
          } yield resp
      }
  }
    ...
    val authEndpoints: AuthService[F, Auth] = {
      val allRoles =
        createPetEndpoint(petService)
          ...
          .orElse(getPetsByStatusEndpoint(petService))
          .orElse(getPetsByTagEndpoint(petService))
      val onlyAdmin =
